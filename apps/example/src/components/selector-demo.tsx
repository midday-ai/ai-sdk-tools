...
import { useChatError, useChatProperty, useChatStatus } from "ai-sdk-zustand";
...
export function SelectorDemo() {
  ...
  const latestMessage = useChatProperty<
    CustomUIMessage,
    CustomUIMessage | undefined
  >((state) =>
  ...
  const toolCallCount = useChatProperty<CustomUIMessage, number>((state) => {
  ...
  const messagesWithMetadata = useChatProperty<
    CustomUIMessage,
    CustomUIMessage[]
  >((state) =>
    ...
    <div className="bg-black text-white font-mono text-sm rounded-lg overflow-hidden border border-gray-800">
      ...
      <div className="bg-gray-900 px-4 py-2 border-b border-gray-800 flex items-center gap-2">
        ...
        <div className="text-gray-400 text-xs ml-2">
          ai-sdk-zustand@latest selector-demo
        </div>
      </div>
      ...
      <div className="p-4 space-y-4">
        <div>
          ...
          <span className="text-white">npx ai-sdk-zustand@latest demo</span>
        </div>
        ...
        <div className="space-y-1">
          ...
          <div className="ml-2 space-y-0.5">
            ...
            <div>
              │ User:{" "}
              <span className="text-cyan-400">{userMessages.length}</span>
            </div>
            <div>
              │ Assistant:{" "}
            ...
            <div>
              ...
              <span
                className={
                  status === "streaming" ? "text-yellow-400" : "text-gray-400"
                }
              >
                {status === "streaming" ? "●" : "○"}
              </span>
            ...
            <div>
              │ Tool Calls:{" "}
              <span className="text-orange-400">{toolCallCount}</span>
            </div>
          </div>
        ...
        {latestMessage && (
          <div className="space-y-1">
            ...
            <div className="ml-2 space-y-0.5">
              <div>
                │ Role:{" "}
                <span className="text-green-400">{latestMessage.role}</span>
              </div>
              <div>
                │ Parts:{" "}
              ...
              <div>
                ...
                <span
                  className={
                    ...
                    Object.keys(latestMessage.metadata).length > 0
                      ? "text-green-400"
                      : "text-gray-400"
                  }
                >
        ...
        {messagesWithMetadata.length > 0 && (
          <div className="space-y-1">
            <div className="text-gray-400">
              ◇ Messages with Metadata ({messagesWithMetadata.length})
            </div>
            <div className="ml-2 max-h-32 overflow-y-auto space-y-1">
              {messagesWithMetadata.slice(0, 3).map((message, index) => (
                <div key={message.id}>
                  <div>
                    │ {index + 1}.{" "}
                    <span className="text-cyan-400">{message.role}</span>{" "}
                    message
                  </div>
                  <div className="text-gray-500 text-xs ml-2">
                    │ {Object.keys(message.metadata || {}).join(", ")}
                  </div>
                </div>
              ))}
              {messagesWithMetadata.length > 3 && (
                <div className="text-gray-500">
                  │ ... and {messagesWithMetadata.length - 3} more
                </div>
              )}
            </div>
          ...
          <div className="ml-2 space-y-1">
            ...
            <div className="ml-2 text-xs space-y-0.5 text-gray-300">
              <div>
                ...
                <span className="text-yellow-400">useChatMessages</span>()
              </div>
              <div>
                │ <span className="text-blue-400">const</span> status ={" "}
                <span className="text-yellow-400">useChatStatus</span>()
              </div>
              <div>
                │ <span className="text-blue-400">const</span> isLoading ={" "}
                <span className="text-yellow-400">useChatIsLoading</span>()
              </div>
            </div>
            ...
            <div className="ml-2 text-xs text-gray-300">
              <div>
                ...
                <span className="text-yellow-400">useChatProperty</span>(state
